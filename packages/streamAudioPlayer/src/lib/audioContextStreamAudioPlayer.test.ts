...
describe('AudioContextStreamAudioPlayer', () => {
  ...
  describe('playNextBuffer', () => {
    ...
    it('should disconnect previous source node before playing next', () => {
      ...
      expect(mockSourceNode.removeEventListener).toHaveBeenCalledWith('ended', expect.any(Function));
    });
    ...
    it('should bind ended event to play next buffer', () => {
      ...
      expect(mockSourceNode.addEventListener).toHaveBeenCalledWith('ended', expect.any(Function));
    });
  });
